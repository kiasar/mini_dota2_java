...
public class GameObjectID implements java.io.Serializable {
    ...
    public static GameObjectID create(Class type,Item item) {
        GameObjectID id=new GameObjectID(type, NEXT++,item);
        ids.add(id);
        return id;
    }

    public static GameObjectID getObject(int number){
        for (GameObjectID gameObjectID :ids) {
            if(gameObjectID.number==number)return gameObjectID;
        }
        return null;
    }
    ...
    public static GameObjectID reconstruct(String id) {
        ...
        try {
            return new GameObjectID(Class.forName(className), n,null);
    ...
    private GameObjectID(Class type, int number,Item item) {
        this.type = type;
        this.number = number;
        this.item=item;
    }
